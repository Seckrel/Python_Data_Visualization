...
def make_dict(table, key_col):
    ...
    return {row.pop(key_col): row for row in temp_table}
...
def merge_csv_files(cancer_csv_file, center_csv_file, joined_csv_file):
    ...
    joined_list = [[key] + cancer_dict[key] + center_dict[key] for key in cancer_dict.keys() if key in center_dict.keys()]
    write_csv_file(joined_list, joined_csv_file)

    # Print warning about FIPS codes in USA map that are missing from cancer risk data
    pass


merge_csv_files("./Datasets/cancer_risk_trimmed_solution.csv", "./Datasets/USA_Counties_with_FIPS_and_centers.csv", "./Datasets/cancer_risk_joined.csv")
