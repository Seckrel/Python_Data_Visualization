...
def reconcile_countries_by_name(plot_countries, gdp_countries):
    ...
    for key in plot_countries:
        if plot_countries[key] in gdp_countries.keys():
            matching[key] = plot_countries[key]
        else:
...
def build_map_dict_by_name(gdpinfo, plot_countries, year):
    ...
    matching, not_matching = reconcile_countries_by_name(plot_countries, table)
    not_available = set()
    available = {}
    for key in matching:
        temp_data = table[matching[key]][year]
        if temp_data != '':
            available[key] = math.log10(float(temp_data))
        else:
            not_available.add(key)

    return available, not_matching, not_available
...
def render_world_map(gdpinfo, plot_countries, year, map_file):
    ...
    available, not_matching, not_available = build_map_dict_by_name(gdpinfo, plot_countries, year)
    worldmap_chart = pygal.maps.world.World()
    worldmap_chart.title = f"GDP by country for {year} (log scale), unified by common country Name"
    worldmap_chart.add("GDP for "+year, available)
    worldmap_chart.add("Missing from World Bank Data", not_matching)
    worldmap_chart.add("No GDP data", not_available)
