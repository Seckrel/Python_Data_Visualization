...
def build_plot_values(gdpinfo, gdpdata):
    ...
    for key in gdpdata:
        try:
            temp_key = int(key)
            if min_year <= temp_key <= max_year:
                temp_val = float(gdpdata[key])
                gdp.append((temp_key, temp_val))
        except ValueError:
            continue

    return sorted(gdp, key=lambda x: x[0])
...
def build_plot_dict(gdpinfo, country_list):
    ...
    for name in country_list:
        try:
            coordinates[name] = build_plot_values(gdpinfo, table[name])
        except KeyError:
            coordinates[name] = []
...
def render_xy_plot(gdpinfo, country_list, plot_file):
    ...
    """
    coordinate = build_plot_dict(gdpinfo, country_list)
    gdp_chart = pygal.XY()
    ...
    for country in coordinate:
        gdp_chart.add(country, coordinate[country])
